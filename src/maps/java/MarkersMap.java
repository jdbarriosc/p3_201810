 ...
 */
public class MarkersMap extends MapView {
	public MarkersMap() {
		...
		setOnMapReadyHandler(new MapReadyHandler() {
			...
			public void onMapReady(MapStatus status) {
				...
				if (status == MapStatus.MAP_STATUS_OK) {
					...
					GrafoDirigido grafo=TaxiTripsManagerView.darComponenteMayor().darGrafo();
					String color=TaxiTripsManagerView.darComponenteMayor().darColor();
					String color2=TaxiTripsManagerView.darComponenteMayor().darColor();
					if(TaxiTripsManagerView.darComponenteMayor().darColor().length()<6)
					{
						color2="0"+color2;
					}
					color="#"+color2;
					...
					Iterator<VerticeLatLongServicios>iter=grafo.darVertices().iterator();
	...
	public static void mostrar() {
		final MarkersMap sample = new MarkersMap();

		JFrame frame = new JFrame("Componente Conexa");

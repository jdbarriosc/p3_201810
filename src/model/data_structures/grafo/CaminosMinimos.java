...
public class CaminosMinimos<K, V extends IVertice<K>, A extends IArco>
{
     ...
     * @param pVertices Vértices que hacen parte del grafo al que pertenece el vértice origen
     */
    public CaminosMinimos( int referencia,Vertice<K, V, A> pOrigen, Collection<Vertice<K, V, A>> pVertices )
    {
        // Guarda el origen de todos los caminos mínimos
        origen = pOrigen;
        // Crea la estructura que representa la información del camino mínimo asociada con cada vértice
        ...
        for( Arco<K, V, A> arco : origen.darSucesores( ) )
        {
            asignarCosto( arco.darVerticeDestino( ).darId( ), arco.darPeso(referencia ), nodoInicial );
        }
        // Inicializa el costo del camino especial al vértice de origen como de costo 0
        asignarCosto( origen.darId( ), 0, null );
        origen.marcar( );
     ...
     */
    public void asignarCosto( K idVertice, double costo, NodoDijkstra<K, V, A> anterior )
    {
    ...
    public void recalcularCaminosEspeciales( Vertice<K, V, A> nuevoVert )
    {
        ...
        double costoANuevoVert = nuevoNodo.darCostoMinimo( );
     ...
     * @param nuevoVert Nuevo vértice que se acaba de incluir en el camino
     */
    public void recalcularCaminosEspeciales( int referencia,Vertice<K, V, A> nuevoVert )
    {
        NodoDijkstra<K, V, A> nuevoNodo = nodos.get( nuevoVert.darId( ) );
        double costoANuevoVert = nuevoNodo.darCostoMinimo( );
        for( Arco<K, V, A> arco : nuevoVert.darSucesores( ) )
        {
            ...
            {
                NodoDijkstra<K, V, A> nodo = nodos.get( vert.darId( ) );
                if( nodo.darCostoMinimo( ) > costoANuevoVert + arco.darPeso(referencia ) )
                    nodo.asignarCostoMinimo( costoANuevoVert + arco.darPeso(referencia), nuevoNodo );
            }
        }
    }
     ...
     */
    public double darCostoCamino( Vertice<K, V, A> vertice )
    {
        NodoDijkstra<K, V, A> nodo = nodos.get( vertice.darId( ) );
        double costo = nodo.darCostoMinimo( );
     ...
     */
    public Iterador<V> darCaminoMinimo( Vertice<K, V, A> vertice )
    {
        NodoDijkstra<K, V, A> nodo = nodos.get( vertice.darId( ) );
        IteradorSimple<V> itera = new IteradorSimple<V>( nodos.size( ) );
        try
        {
            while( nodo != null )
            {
                itera.insertar( nodo.darVertice( ).darInfoVertice() );
